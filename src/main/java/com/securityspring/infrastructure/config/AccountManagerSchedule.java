...
public class AccountManagerSchedule {
    ...
    public void runDailyDeletion() {
        ...
        UserVO user = this.loginService.getUserByUsername("admin");
        int deletedCount = this.logService.deleteOldLogs();
        this.logService.setLog("DAILY SCHEDULE", user.getIdentifier(), String.format("Daily log deletion completed. Total logs deleted: %d",
                deletedCount));
